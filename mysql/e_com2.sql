...
-- Add total_orders column directly to the customers table first
ALTER TABLE `customers` ADD COLUMN `total_orders` INT DEFAULT 0;

-- Update stored procedure without ALTER TABLE to avoid implicit commit
DELIMITER //
CREATE PROCEDURE update_customer_total_orders(IN cust_id INT)
BEGIN
    -- Update total orders for customer
    UPDATE customers 
    SET total_orders = (
        SELECT COUNT(*) 
        FROM customer_order 
...
-- Define the trigger without transaction control
DELIMITER //
CREATE TRIGGER after_order_insert 
AFTER INSERT ON customer_order
FOR EACH ROW
BEGIN
    CALL update_customer_total_orders(NEW.customer_id);
END//
DELIMITER ;


-- Function to calculate customer total spending
DELIMITER //
CREATE FUNCTION calculate_customer_total(cust_id INT) 
...
BEGIN
    ...
    SELECT SUM(due_amount) INTO total
    FROM customer_order
    WHERE customer_id = cust_id
...
-- Stored procedure to get customer order history with product details
DELIMITER //
