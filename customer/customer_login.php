...
if (isset($_POST['login'])) {
	...
	// Fetch customer details if login is successful
	$row_customer = mysqli_fetch_assoc($run_cust);
	$customer_id = $row_customer['customer_id']; // Retrieve customer_id
	$_SESSION['customer_id'] = $customer_id; // Set customer_id in session
	echo "Customer ID: " . $_SESSION['customer_id']; // Debug line
	$total_orders = $row_customer['total_orders'];
	$_SESSION['total_orders'] = $total_orders; // Set customer_id in session
	echo "tot ord: " . $_SESSION['total_orders'];
